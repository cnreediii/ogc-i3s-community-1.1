...
This section is comprised of two main sub-sections: A general tiling conceptual model and a detailed logical model for tiling 2D Euclidean Space. The conceptual model is very abstract and cannot be implemented as is. A One or more logical models are required to provide the requirements and structure necessary to move to implementation.
...
`*tile*` The fundamental unit when partitioning space is the `tile`. A `tile` is a geometric shape with known properties that is the result of the tiling (tessellation) of a plane. A tile consists of a single connected "piece" without "holes" or "lines" (topological disc). In two dimensional space, a tile could be regular (e.g. square) or irregular (e.g. Thiesien polygon). A voxel is an example of a regular partitioning of 3 dimensional space.
